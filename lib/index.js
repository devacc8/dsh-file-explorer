        ...
        route('/plugins/file-explorer/open-folder', async (req, res) => {
            ...
            try {
                ...
                const plan = () => {
                    ...
                    if (isWsl())
                        list.push({ program: 'explorer.exe', args: [wslWindowsPath(display)], detached: true });
                    list.push({ program: 'xdg-open', args: [parent] }, { program: 'gio', args: ['open', parent] }, { program: 'exo-open', args: ['--launch', 'FileManager', parent] }, { program: 'kde-open5', args: [parent] }, { program: 'kde-open', args: [parent] }, { program: 'nautilus', args: [parent] }, { program: 'dolphin', args: [parent] }, { program: 'thunar', args: [parent] }, { program: 'pcmanfm', args: [parent] }, { program: 'nemo', args: [parent] }, { program: 'caja', args: [parent] });
                    return list;
                };
                const tried = [];
                for (const candidate of plan()) {
                    let resolved = null;
                    try {
                        resolved = await subprocess.resolveExecutable(candidate.program);
                    }
                    catch { /* not on PATH */ }
                    if (resolved === null || resolved === undefined) {
