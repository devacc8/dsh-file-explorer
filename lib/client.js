...
`;
		...
		const ExplorerPanel = (props) => {
			...
			// id with it, so both selectors this used to read returned undefined and the
			// tree fell through to the first workspace, which pinned it to whichever
			// project was opened first. The session in the main view is the one that view
			// retains, which is how the harness resolves it too.
			const currentSessionId = props.useSessions((st) => Object.values(st.byId).find((s) => ((s.retainedBy && s.retainedBy.mainView) || 0) > 0)?.id ?? null);
