...
`;
		...
		const sortEntries = (entries, key) => {
			...
			return entries.slice().sort((a, b) => {
				...
				const cmp = String(a.name).localeCompare(String(b.name), void 0, { numeric: true, sensitivity: "base" });
				return mode === "name-desc" ? -cmp : cmp;
			});
		};
		const fileTypeOf = (name) => {
			const lower = String(name).toLowerCase();
			for (const entry of FILE_NAMES) if (entry.names.indexOf(lower) !== -1) return { tone: entry.tone, icon: entry.icon, fillRule: entry.fillRule };
			const ext = lower.split(".").pop();
			for (const type of FILE_TYPES) if (type.exts.indexOf(ext) !== -1) return { tone: type.tone, icon: type.icon || "file", fillRule: type.fillRule };
			return { tone: "default", icon: "file", fillRule: void 0 };
		};
		const fmtSize = (n) => {
			...
			const expandAll = () => {
				...
				work(tree.rootPath).then(() => {
					...
					if (loaded >= MAX_EXPAND_DIRS) showStatus({
						ok: false,
						text: "Many directories; expanded the first 500"
					});
				}).catch(() => {
					expandBusy = false;
				});
