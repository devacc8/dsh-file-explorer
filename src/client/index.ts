...
`;
		...
		const mdInline = (s) => {
			...
			t = t.replace(/(?<!!)\[([^\]]+)\]\(([^)\s]+)\)/g, (m, txt, href) => /^(?:javascript|data|vbscript|blob|file):/i.test(String(href).replace(/[\u0000-\u0020\u007f]+/g, '')) ? '<span>' + txt + '</span>' : '<a href="' + href + '" target="_blank" rel="noreferrer">' + txt + '</a>');
			return t;
		};
