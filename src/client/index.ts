...
`;
		...
		const TreeNode = (props) => {
			...
			const row = react.createElement('div', {
				...
				react.createElement('span', { className: 'fe-chevron' + (isDir ? '' : ' fe-chevron-none') }, isDir
					? react.createElement(Icon, { name: expanded ? 'chevronDown' : 'chevronRight', size: 13 })
					: null),
				react.createElement('span', { className: 'fe-node-icon fe-tone-' + (isDir ? 'dir' : fileType.tone) },
					react.createElement(Icon, { name: isDir ? 'folder' : fileType.icon, size: 15, fillRule: isDir ? undefined : fileType.fillRule })),
		...
		const ExplorerPanel = (props) => {
			...
			if (!rootPath && sessionCwd) {
				// The session's own directory is the workspace root even while the
				// workspace list is still catching up with a brand new project.
				rootPath = sessionCwd;
			...
			const renderTree = () => {
				...
				rows.push(react.createElement('div', {
					...
					react.createElement('span', { className: 'fe-chevron' }, react.createElement(Icon, { name: tree.expanded.has(tree.rootPath) ? 'chevronDown' : 'chevronRight', size: 13 })),
					react.createElement('span', { className: 'fe-node-icon fe-tone-dir' }, react.createElement(Icon, { name: 'folder', size: 15 })),
			...
			const renderSearch = () => {
				...
				for (const m of s.matches) {
					...
					rows.push(react.createElement('div', {
						...
						react.createElement('span', { className: 'fe-node-icon fe-tone-' + ft.tone }, react.createElement(Icon, { name: ft.icon, size: 15, fillRule: ft.fillRule })),
