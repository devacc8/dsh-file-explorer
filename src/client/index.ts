...
.fe-status { padding: 4px 10px; font-size: 12px; flex: none; }
...
.fe-node-loading { color: var(--dsw-alias-label-secondary); font-size: 12px; }
.fe-node-error { color: var(--dsw-alias-state-error-primary); font-size: 13px; padding: 4px 8px; }
.fe-empty { color: var(--dsw-alias-label-secondary); padding: 14px 10px; font-size: 13px; }
.fe-preview {
...
.fe-editor-path { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; }
.fe-btn {
...
.fe-editor-msg { padding: 10px 12px; font-size: 13px; color: var(--dsw-alias-label-secondary); }
...
.fe-md h1 { font-size: 22px; margin: 10px 0 6px; }
.fe-md h2 { font-size: 19px; margin: 10px 0 6px; }
.fe-md h3 { font-size: 17px; margin: 8px 0 4px; }
.fe-md h4, .fe-md h5, .fe-md h6 { font-size: 15px; margin: 8px 0 4px; }
...
.fe-md table { border-collapse: collapse; margin: 8px 0; width: 100%; font-size: 13.5px; }
...
.fe-md table code { font-size: 12.5px; }
...
`;
		...
		// Every request carries the plugin header the host requires.
		...
		const api = {
			...
			write: (path, content) => fetch('/plugins/file-explorer/write', {
				method: 'POST',
				headers: { 'content-type': 'application/json', ...PLUGIN_HEADERS },
		...
		const mdInline = (s) => {
			let t = escapeHtml(s);
			// Inline code must stay literal, so its spans are parked in NUL
		...
		const ExplorerPanel = (props) => {
			...
			const onEditClick = () => {
				...
				if (editor && editor.state === 'ready') {
					...
					else showStatus({ ok: false, text: 'Select a file in the tree to edit' });
				} else showStatus({ ok: false, text: 'Select a file in the tree to edit' });
			};
			...
			const onSave = () => {
				...
				api.write(path, content).then((res) => {
					if (res && res.error) showStatus({ ok: false, text: 'Save failed: ' + res.error });
					else {
						updateTab(path, (t) => ({ ...t, editing: false }));
						showStatus({ ok: true, text: 'Saved' });
					}
				}).catch((err) => showStatus({ ok: false, text: 'Save failed: ' + String((err && err.message) || err) }));
			};
			...
			const expandAll = () => {
				...
				work(tree.rootPath).then(() => {
					...
					if (loaded >= MAX_EXPAND_DIRS) {
						showStatus({ ok: false, text: 'Many directories; expanded the first ' + MAX_EXPAND_DIRS + ' directories' });
					}
			...
			const onVscode = () => {
				...
				api.openVscode(tree.rootPath).then((res) => {
					showStatus(res && res.ok
						? { ok: true, text: 'Opened project in VS Code' }
						: { ok: false, text: (res && res.error) || 'Open failed (code command not found)' });
				}).catch((err) => showStatus({ ok: false, text: 'Open failed: ' + String((err && err.message) || err) }));
			};
			...
			const onOpenFolder = () => {
				...
				api.openFolder(target).then((res) => {
					showStatus(res && res.ok
						? { ok: true, text: tree.selected ? 'Revealed selection in system file manager' : 'Opened project in system file manager' }
						: { ok: false, text: (res && res.error) || 'Open failed' });
				}).catch((err) => showStatus({ ok: false, text: 'Open failed: ' + String((err && err.message) || err) }));
			};
			...
			const renderSearch = () => {
				...
				if (s.truncated) rows.push(react.createElement('div', { key: 'trunc', className: 'fe-node-error' }, 'Too many results; truncated (first 300)'));
				return rows;
			};
			...
			const renderPreview = () => {
				...
				const head = react.createElement('div', { className: 'fe-editor-head' },
					...
					isMd && editor.state === 'ready' && !editor.editing
						? react.createElement('button', { className: 'fe-btn', onClick: () => updateTab(editor.path, (t) => ({ ...t, preview: !t.preview })) }, editor.preview ? 'Source' : 'Preview')
						: null,
					editor.state === 'ready' && editor.editing
						? react.createElement('button', { className: 'fe-btn', onClick: onSave }, 'Save')
						: null,
					react.createElement('button', { className: 'fe-btn', onClick: () => { closePreviewPane(); setStatus(null) } }, 'Collapse preview'),
				);
				let body = null;
			...
			const treePanel = s.open ? react.createElement('div', { className: 'fe-panel', style: { width: s.width + 'px' } },
				react.createElement('div', { className: 'fe-resize', title: 'Drag to resize', onPointerDown: (e) => onResizeStart('tree', e) }),
				treeTab,
				react.createElement('div', { className: 'fe-header' },
					react.createElement('span', { className: 'fe-title' }, 'Files'),
					react.createElement('button', { className: 'fe-iconbtn fe-icon-vscode', title: 'Open project in Visual Studio Code', onClick: onVscode }, react.createElement(Icon, { name: 'vscode', size: 15 })),
				...
				react.createElement('div', { className: 'fe-searchbar' },
					react.createElement('input', { className: 'fe-search', type: 'text', placeholder: 'Search files', value: s.query, spellCheck: false, onChange: (e) => setQuery(e.target.value) }),
			...
			const previewPane = editor
				? react.createElement('div', {
					...
					react.createElement('div', { className: 'fe-preview-resize', title: 'Drag to resize', onPointerDown: (e) => onResizeStart('preview', e) }),
					previewTab,
					renderTabBar(),
					renderPreview(),
				)
		...
		function apply(ctx) {
			...
			slots.inject('shell.overlay', () => slots.register(
				{ name: 'shell.overlay', id: 'file-explorer', order: 90, label: 'File Explorer' },
				(props) => react.createElement(ExplorerPanel, props),
			));
			slots.inject('conversation.session.header.actions', () => slots.register(
				{ name: 'conversation.session.header.actions', id: 'file-explorer-toggle', order: 30, label: 'File Explorer' },
